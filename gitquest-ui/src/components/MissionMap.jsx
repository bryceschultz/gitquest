      ...
      title={
        clickable
          ? levelIsDone
            ? "Replay this lesson"
            : "Open this lesson"
          : "Locked — complete the previous lessons first"
      }
      style={{
        display: "flex",
        alignItems: "center",
        justifyContent: "space-between",
        background: levelIsDone
          ? "#0d1f15"
          : fieldAssignment
            ? "#141005"
            : "#080c17",
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <span
          style={{
            fontSize: 12,
            color: levelIsDone ? "#00ff88" : "#2a3a55",
            fontFamily: "monospace",
            minWidth: 16,
          }}
        >
        ...
        >
            ...
            quest.assignments ?? [
              {
                id: `${quest.id}-all`,
                title: null,
                lessons: Object.keys(quest.levels).filter(
                  (id) => id !== quest.fieldAssignment,
                ),
              },
            ]
          ).map((assignment) => (
          ...
          {quest.fieldAssignment && quest.levels[quest.fieldAssignment] && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <LevelRow
                level={quest.levels[quest.fieldAssignment]}
                questId={quest.id}
                isLevelComplete={isLevelComplete}
                progress={progress}
                onStartLevel={onStartLevel}
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <button
            style={{
              ...
              borderRadius: 8,
              padding: "10px 20px",
              cursor: startLevelId ? "pointer" : "not-allowed",
              fontFamily: "monospace",
              fontSize: 13,
              fontWeight: 500,
            }}
          ...
          {missionDone && (
            ...
            >
              ✓ mission complete — all lessons replayable
            </span>
          )}
        </div>
      </div>
...
export default function MissionMap({
  ...
  const centers = MISSION_ORDER.map((id) => ({
    id,
    x: MISSIONS[id].x + 20,
    y: MISSIONS[id].y + 20,
  }));
  const lines = centers
    .slice(0, -1)
    .map((c, i) => [c.x, c.y, centers[i + 1].x, centers[i + 1].y]);
    ...
    >
      ...
      >
        ...
        <span
          ...
          style={{ fontSize: 12, color: "#c8daf0", fontFamily: "monospace" }}
        >
          🎖 {missionsDone}/{MISSION_ORDER.length}
        </span>
        <span
          title="Coin balance"
          style={{ fontSize: 12, color: "#c8daf0", fontFamily: "monospace" }}
      ...
      <div style={{ flex: 1, padding: "1.5rem", position: "relative" }}>
        ...
        {!isRecruit && recommended && (
          ...
          >
            📋 placement result: {progress.placement.pct}% — recommended start:{" "}
            {recommended} — {MISSIONS[recommended]?.title.split("— ")[1]}
          </div>
        )}
        ...
        >
          {[
            { label: "Trophy room", icon: "🏆", action: onOpenTrophy },
            { label: "Arsenal", icon: "🔧", action: onOpenArsenal },
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
            ...
            <button
              onClick={() => {
                if (
                  window.confirm(
                    "Reset ALL GitQuest progress? This cannot be undone.",
                  )
                )
                  resetProgress();
              }}
        ...
        >
          ...
          {MISSION_ORDER.map((id) => {
            ...
            return (
              <g
                key={q.id}
                onClick={() => handleQuestClick(q, locked)}
